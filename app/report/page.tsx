...
export default function ReportPage() {
  ...
  const analyzeReport = async () => {
    ...
    try {
      ...
      setResult(t("Report analyse nahi ho paya. Dobara try karein."));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      <div style={{ maxWidth: 820, margin: "0 auto", padding: "32px clamp(20px,4vw,40px) 100px" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 28 }}>
          ...
          <div>
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--text-3)", letterSpacing: "0.08em", textTransform: "uppercase" }}>{t("Blood test, X-ray, MRI, ECG · Hindi mein explain")}</div>
            <h1 style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "clamp(26px,4vw,38px)", letterSpacing: "-0.025em", color: "#F0F4FF", margin: "6px 0 0" }}>{t("Report Analyser")}</h1>
          </div>
        </div>
        ...
        {!result && (
          ...
          >
            ...
            <div style={{ fontSize: 18, fontWeight: 700, color: "#F0F4FF", fontFamily: "var(--font-display)", marginBottom: 8 }}>{t("Report Upload Karein")}</div>
            <div style={{ fontSize: 13, color: "var(--text-3)", lineHeight: 1.7 }}>
              {t("Blood test, X-ray, MRI, ECG, Urine report")}<br />
              {t("Drag & drop ya click karke select karein")}<br />
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 11 }}>{t("JPG, PNG, PDF · max 10MB")}</span>
            </div>
          </div>
        )}
        ...
        {file && !result && (
          <GlassCard accent="#00B4D8" lift={false} style={{ padding: 20, marginBottom: 20 }}>
            ...
            {preview && <img src={preview} alt={t("Report preview")} style={{ width: "100%", borderRadius: 12, maxHeight: 300, objectFit: "contain", background: "#000" }} />}
          </GlassCard>
        )}
        ...
        {file && !result && (
          <button onClick={analyzeReport} disabled={loading} style={{ width: "100%", background: loading ? "rgba(255,255,255,0.04)" : "linear-gradient(135deg,#00E676,#00C4FF)", border: "none", borderRadius: 100, padding: "16px 24px", fontSize: 16, fontWeight: 700, color: loading ? "var(--text-3)" : "#04060D", cursor: loading ? "not-allowed" : "pointer", fontFamily: "var(--font-body)", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, transition: "all 0.2s" }}>
            {loading ? (
              <><div style={{ width: 18, height: 18, border: "2px solid rgba(255,255,255,0.2)", borderTopColor: "#00E676", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />{t("AI Report Padh raha hai...")}</>
            ) : <><Search size={18} color="#04060D" strokeWidth={2} />{t("AI Se Analyse Karwayein")}</>}
          </button>
        )}
          ...
          <div style={{ animation: "fadeUp 0.5s ease forwards" }}>
            <GlassCard accent={riskColor} lift={false} style={{ padding: 24, marginBottom: 20 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
                ...
                <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: riskColor, letterSpacing: "0.08em", textTransform: "uppercase" }}>{t("AI ANALYSIS COMPLETE")}</span>
              </div>
            ...
            <div style={{ background: "rgba(255,71,87,0.06)", border: "1px solid rgba(255,71,87,0.15)", borderRadius: 14, padding: "14px 18px", marginBottom: 16, fontSize: 13, color: "var(--text-2)", lineHeight: 1.7 }}>
              <AlertTriangle size={14} color="#FF4757" strokeWidth={1.8} style={{ display: "inline", verticalAlign: "-2px", marginRight: 6 }} /><strong style={{ color: "#F0F4FF" }}>{t("Disclaimer:")}</strong> {t("Yeh AI analysis hai. Final diagnosis ke liye doctor se milein. Koi bhi urgent value ko ignore mat karein.")}
            </div>

            <div style={{ display: "flex", gap: 12 }}>
              <button onClick={() => { setFile(null); setPreview(null); setResult(null); }} style={{ flex: 1, background: "rgba(255,255,255,0.04)", border: "1px solid var(--border)", borderRadius: 100, padding: "12px 20px", color: "#F0F4FF", cursor: "pointer", fontFamily: "var(--font-body)", fontSize: 14 }}>{t("Nayi Report")}</button>
              <button onClick={() => router.push("/chat")} style={{ flex: 1, background: "linear-gradient(135deg,#00E676,#00C4FF)", border: "none", borderRadius: 100, padding: "12px 20px", color: "#04060D", cursor: "pointer", fontFamily: "var(--font-body)", fontSize: 14, fontWeight: 700 }}>{t("Doctor Se Poochhein")} →</button>
            </div>
          </div>
        )}
          ...
          <div className="m-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginTop: 8 }}>
            {([[Droplet, "Blood Test", "CBC, LFT, KFT, Lipid", "#FF4757"], [Scan, "X-Ray", "Chest, Bone, Dental", "#00B4D8"], [Dna, "MRI/CT Scan", "Brain, Spine, Abdomen", "#a78bfa"], [Heart, "ECG", "Heart rhythm analysis", "#FF4757"], [Heart, "Urine Test", "Routine, Culture", "#fbbf24"], [Pill, "Prescription", "Medicine explanation", "#00E676"]] as const).map(([Icon, title, sub, accent]) => (
              <GlassCard key={title} accent={accent} lift={false} style={{ padding: 16, textAlign: "center" }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 600, color: "#F0F4FF", marginBottom: 4 }}>{t(title)}</div>
                <div style={{ fontSize: 11, color: "var(--text-3)", fontFamily: "var(--font-mono)" }}>{t(sub)}</div>
              </GlassCard>
            ))}
          </div>
        )}
