...
export default function MedicinePage() {
  ...
  const analyze = async () => {
    ...
    } catch { setResult("Medicine identify nahi ho paayi. Dobara try karein."); }
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      <div style={{ maxWidth: 820, margin: "0 auto", padding: "32px clamp(20px,4vw,40px) 100px" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 28 }}>
          ...
          <div>
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--text-3)", letterSpacing: "0.08em", textTransform: "uppercase" }}>Photo lo ya naam likho · AI sab batayega</div>
            <h1 style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "clamp(26px,4vw,38px)", letterSpacing: "-0.025em", color: "#F0F4FF", margin: "6px 0 0" }}>Medicine Scanner</h1>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 24, background: "rgba(255,255,255,0.025)", border: "1px solid var(--border)", borderRadius: 14, padding: 6 }}>
          {(["photo", "text"] as const).map(m => (
            <button key={m} onClick={() => { setMode(m); setResult(null); setFile(null); setPreview(null); setMedicineName(""); }} style={{ flex: 1, padding: "10px", borderRadius: 100, border: "none", background: mode === m ? "linear-gradient(135deg,#00E676,#00C4FF)" : "transparent", color: mode === m ? "#04060D" : "var(--text-3)", fontSize: 14, fontWeight: mode === m ? 600 : 400, cursor: "pointer", fontFamily: "var(--font-body)", transition: "all 0.2s", display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}>
              {m === "photo" ? <><Camera size={15} strokeWidth={1.8} /> Photo Se</> : <><Pencil size={15} strokeWidth={1.8} /> Naam Se</>}
            </button>
          ))}
        </div>
        ...
        {!result && (
          <GlassCard accent="#FF4757" lift={false} style={{ padding: "16px 18px", marginBottom: 16 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: "#FF4757", marginBottom: 8, display: "flex", alignItems: "center", gap: 6 }}><AlertTriangle size={14} color="#FF4757" strokeWidth={1.8} /> Aap pehle se koi medicine le rahe hain?</div>
            <input value={currentMeds} onChange={e => setCurrentMeds(e.target.value)} placeholder="e.g. Metformin, BP ki dawai, Aspirin... (interaction check ke liye)" style={{ width: "100%", background: "rgba(255,255,255,0.025)", border: "1px solid var(--border)", borderRadius: 10, padding: "10px 12px", color: "#F0F4FF", fontSize: 13, fontFamily: "var(--font-body)", outline: "none" }} />
            <div style={{ fontSize: 11, color: "var(--text-3)", marginTop: 6, lineHeight: 1.5 }}>Likhne se AI nayi medicine ke saath dangerous interaction check karega.</div>
          </GlassCard>
        )}
          ...
          <div>
            ...
            {!file ? (
              <div onClick={() => fileRef.current?.click()} style={{ border: "2px dashed var(--border)", borderRadius: 20, padding: "60px 40px", textAlign: "center", cursor: "pointer", marginBottom: 20, background: "rgba(255,255,255,0.025)" }}>
                ...
                <div style={{ fontSize: 17, fontWeight: 600, color: "#F0F4FF", fontFamily: "var(--font-display)", marginBottom: 8 }}>Medicine Ki Photo Lo</div>
                <div style={{ fontSize: 13, color: "var(--text-3)", lineHeight: 1.7 }}>Strip, tablet, ya packaging ki clear photo<br /><span style={{ fontFamily: "var(--font-mono)", fontSize: 11 }}>Camera se seedha ya gallery se choose karein</span></div>
              </div>
            ) : (
              <div style={{ marginBottom: 20 }}>
                {preview && <img src={preview} alt="Medicine" style={{ width: "100%", borderRadius: 16, maxHeight: 280, objectFit: "contain", background: "#000", marginBottom: 12 }} />}
                <div style={{ display: "flex", gap: 10 }}>
                  <button onClick={() => { setFile(null); setPreview(null); }} style={{ flex: 1, background: "rgba(255,255,255,0.025)", border: "1px solid var(--border)", borderRadius: 100, padding: "12px", color: "var(--text-2)", cursor: "pointer", fontFamily: "var(--font-body)" }}>Dobara Lo</button>
                  <button onClick={analyze} disabled={loading} style={{ flex: 2, background: loading ? "rgba(255,255,255,0.025)" : "linear-gradient(135deg,#00E676,#00C4FF)", border: "none", borderRadius: 100, padding: "12px", color: loading ? "var(--text-3)" : "#04060D", cursor: loading ? "not-allowed" : "pointer", fontFamily: "var(--font-body)", fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                    {loading ? <><div style={{ width: 16, height: 16, border: "2px solid rgba(0,0,0,0.2)", borderTopColor: "#00E676", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />Scan kar raha hai...</> : <><Search size={16} strokeWidth={1.8} /> Scan Karein</>}
                  </button>
                </div>
              </div>
            )}
        ...
        {!result && mode === "text" && (
          <div style={{ marginBottom: 20 }}>
            <div style={{ background: "rgba(255,255,255,0.025)", border: "1px solid var(--border)", borderRadius: 16, padding: "14px 18px", marginBottom: 12, display: "flex", gap: 12, alignItems: "center" }}>
              ...
              <input value={medicineName} onChange={e => setMedicineName(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && medicineName.trim()) analyze(); }} placeholder="Medicine ka naam likhein... (e.g. Paracetamol, Crocin, Metformin)" style={{ flex: 1, background: "transparent", border: "none", outline: "none", color: "#F0F4FF", fontSize: 15, fontFamily: "var(--font-body)" }} autoFocus />
            </div>
            <button onClick={analyze} disabled={!medicineName.trim() || loading} style={{ width: "100%", background: medicineName.trim() && !loading ? "linear-gradient(135deg,#00E676,#00C4FF)" : "rgba(255,255,255,0.025)", border: "none", borderRadius: 100, padding: "15px", fontSize: 15, fontWeight: 600, color: medicineName.trim() && !loading ? "#04060D" : "var(--text-3)", cursor: medicineName.trim() && !loading ? "pointer" : "not-allowed", fontFamily: "var(--font-body)", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
              {loading ? <><div style={{ width: 16, height: 16, border: "2px solid rgba(0,0,0,0.2)", borderTopColor: "#00E676", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />Dhundh raha hai...</> : <><Search size={16} strokeWidth={1.8} /> Medicine Dhundho</>}
            </button>
          </div>
        )}
          ...
          <div style={{ animation: "fadeUp 0.5s ease forwards" }}>
            <GlassCard accent="#00E676" lift={false} style={{ padding: 24, marginBottom: 16 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
                ...
                <span style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "#00E676", letterSpacing: "0.08em" }}>MEDICINE ANALYSIS COMPLETE</span>
              </div>
            ...
            <div style={{ background: "rgba(255,71,87,0.05)", border: "1px solid rgba(255,71,87,0.15)", borderRadius: 14, padding: "12px 16px", marginBottom: 16, fontSize: 12, color: "var(--text-3)", lineHeight: 1.7, fontFamily: "var(--font-mono)", display: "flex", alignItems: "center", gap: 8 }}>
              <AlertTriangle size={14} color="#FF4757" strokeWidth={1.8} style={{ flexShrink: 0 }} /> DOCTOR KI PRESCRIPTION KE BINA KOIN BHI MEDICINE MAT LO
            </div>
            <button onClick={() => router.push("/generic")} style={{ width: "100%", background: "rgba(0,230,118,0.08)", border: "1px solid rgba(0,230,118,0.25)", borderRadius: 100, padding: "12px", color: "#00E676", fontWeight: 600, cursor: "pointer", fontFamily: "var(--font-body)", fontSize: 14, marginBottom: 10, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}><IndianRupee size={16} strokeWidth={1.8} /> Sasta Generic Alternative Dhundho</button>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => { setResult(null); setFile(null); setPreview(null); setMedicineName(""); }} style={{ flex: 1, background: "rgba(255,255,255,0.025)", border: "1px solid var(--border)", borderRadius: 100, padding: "12px", color: "var(--text-2)", cursor: "pointer", fontFamily: "var(--font-body)" }}>Nayi Medicine</button>
              <button onClick={() => router.push("/chat")} style={{ flex: 1, background: "linear-gradient(135deg,#00E676,#00C4FF)", border: "none", borderRadius: 100, padding: "12px", color: "#04060D", fontWeight: 600, cursor: "pointer", fontFamily: "var(--font-body)" }}>Doctor Se Poochhein</button>
            </div>
          </div>
        )}
          ...
          <div className="m-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 8 }}>
            {[[Pill, "Tablet/Capsule", "Strip ya bottle"], [Droplet, "Syrup", "Bottle label"], [Syringe, "Injection", "Vial label"], [Cross, "Ointment", "Tube packaging"]].map(([Icon, title, sub]) => (
              <div key={title as string} style={{ background: "rgba(255,255,255,0.025)", border: "1px solid var(--border)", borderRadius: 14, padding: 14, textAlign: "center" }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 500, color: "#F0F4FF", marginBottom: 3 }}>{title as string}</div>
                <div style={{ fontSize: 11, color: "var(--text-3)", fontFamily: "var(--font-mono)" }}>{sub as string}</div>
              </div>
            ))}
          </div>
        )}
