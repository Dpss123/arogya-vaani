...
export default function DiagnosticsPage() {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      setResult(data.result || "Analysis nahi ho paaya. Dobara try karein.");
    } catch {
      setResult("Analysis nahi ho paaya. Dobara try karein.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      <div style={{ maxWidth: 820, margin: "0 auto", padding: "32px clamp(20px,4vw,40px) 100px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
          <BackButton size={20} />
          <div>
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--text-3)", letterSpacing: "0.08em", textTransform: "uppercase" }}>Photo se screening · diagnosis nahi</div>
            <h1 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(26px,4vw,38px)", fontWeight: 800, letterSpacing: "-0.025em", color: "#F0F4FF", margin: "4px 0 0" }}>AI Diagnostics</h1>
          </div>
        </div>
        ...
        <GlassCard accent="#00B4D8" onClick={() => router.push("/xray")} style={{ padding: "16px 18px", marginBottom: 20, display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: "#00B4D8" }}>Chest X-Ray AI <span style={{ fontSize: 9, padding: "1px 6px", borderRadius: 100, background: "rgba(0,180,216,0.15)", fontFamily: "var(--font-mono)" }}>TRAINED MODEL</span></div>
            <div style={{ fontSize: 11, color: "var(--text-3)", fontFamily: "var(--font-mono)" }}>CheXNet-style pathology detection</div>
          </div>
        ...
        <div className="m-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
          {TYPES.map(t => (
            <button key={t.key} onClick={() => { setType(t.key); setFile(null); setPreview(null); setResult(null); }} style={{ background: type === t.key ? "rgba(167,139,250,0.1)" : "rgba(255,255,255,0.025)", border: `1px solid ${type === t.key ? "rgba(167,139,250,0.4)" : "var(--border)"}`, borderRadius: 14, padding: "14px", cursor: "pointer", textAlign: "left", fontFamily: "var(--font-body)" }}>
              <div style={{ marginBottom: 6 }}><t.Icon size={24} color="#a78bfa" strokeWidth={1.8} /></div>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#F0F4FF" }}>{t.label}</div>
              <div style={{ fontSize: 10, color: "var(--text-3)", fontFamily: "var(--font-mono)" }}>{t.hint}</div>
            </button>
          ))}
        </div>
          ...
          <>
            ...
            {!file ? (
              <div onClick={() => fileRef.current?.click()} style={{ border: "2px dashed rgba(167,139,250,0.25)", borderRadius: 20, padding: "48px 40px", textAlign: "center", cursor: "pointer", background: "rgba(255,255,255,0.025)" }}>
                ...
                <div style={{ fontSize: 16, fontWeight: 600, color: "#F0F4FF", fontFamily: "var(--font-display)", marginBottom: 6 }}>{active.label} ki photo lo</div>
                <div style={{ fontSize: 12, color: "var(--text-3)" }}>Saaf, close-up photo · {active.hint}</div>
              </div>
            ) : (
              <div>
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  ...
                  <button onClick={analyze} disabled={loading} style={{ flex: 2, background: loading ? "rgba(255,255,255,0.025)" : "linear-gradient(135deg,#a78bfa,#00B4D8)", border: "none", borderRadius: 100, padding: "12px", color: loading ? "var(--text-3)" : "#04060D", cursor: loading ? "not-allowed" : "pointer", fontFamily: "var(--font-body)", fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                    {loading ? <><div style={{ width: 16, height: 16, border: "2px solid rgba(0,0,0,0.2)", borderTopColor: "#a78bfa", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />AI dekh raha hai...</> : <><Search size={16} color="#04060D" strokeWidth={2} />Screen Karein</>}
                  </button>
                </div>
              </div>
            )}
        ...
        {result && (
          <div style={{ animation: "fadeUp 0.5s ease forwards" }}>
            <GlassCard accent="#a78bfa" lift={false} style={{ padding: 24, marginBottom: 16 }}>
              <div style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "#a78bfa", letterSpacing: "0.08em", marginBottom: 14, display: "flex", alignItems: "center", gap: 6 }}><active.Icon size={14} color="#a78bfa" strokeWidth={1.8} />AI SCREENING</div>
            ...
            <div style={{ background: "rgba(255,71,87,0.06)", border: "1px solid rgba(255,71,87,0.15)", borderRadius: 14, padding: "12px 16px", marginBottom: 16, fontSize: 12, color: "var(--text-2)", lineHeight: 1.7, display: "flex", gap: 8, alignItems: "flex-start" }}>
              ...
              <span>Yeh AI screening hai · diagnosis nahi. Kisi bhi finding ko doctor se confirm karein.</span>
            </div>
            <button onClick={() => { setFile(null); setPreview(null); setResult(null); }} style={{ width: "100%", background: "rgba(255,255,255,0.025)", border: "1px solid var(--border)", borderRadius: 100, padding: "12px", color: "#F0F4FF", cursor: "pointer", fontFamily: "var(--font-body)" }}>Nayi Photo</button>
          </div>
        )}
      </div>
