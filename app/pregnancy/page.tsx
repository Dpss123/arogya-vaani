...
export default function PregnancyPage() {
  ...
  const getAiAdvice = async () => {
    ...
    try {
      ...
      setAiAdvice(data.advice || t("AI jaankari nahi mil paayi."));
    } catch {
      setAiAdvice(t("AI jaankari nahi mil paayi. Dobara try karein."));
    } finally {
      setAiLoading(false);
    }
  };
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      <div style={{ maxWidth: 820, margin: "0 auto", padding: "32px clamp(20px,4vw,40px) 100px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
          <BackButton size={20} />
          <div>
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--text-3)", letterSpacing: "0.08em", textTransform: "uppercase" }}>{t("Week tracker · Schemes · Danger signs")}</div>
            <h1 style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "clamp(26px,4vw,38px)", letterSpacing: "-0.025em", color: "#F0F4FF", margin: "4px 0 0" }}>{t("Pregnancy Companion")}</h1>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 24, flexWrap: "wrap" }}>
          {([["tracker", "Week Tracker", Calendar], ["schemes", "Govt Schemes", Landmark], ["danger", "Danger Signs", AlertTriangle]] as const).map(([key, label, Icon]) => (
            <button key={key} onClick={() => setTab(key)} style={{ flex: 1, minWidth: 110, padding: "9px 8px", borderRadius: 100, border: `1px solid ${tab === key ? trimesterColor : "var(--border)"}`, background: tab === key ? `${trimesterColor}15` : "transparent", color: tab === key ? trimesterColor : "var(--text-2)", fontSize: 13, cursor: "pointer", fontFamily: "var(--font-body)", fontWeight: tab === key ? 600 : 400, transition: "all 0.2s", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}><Icon size={14} strokeWidth={1.8} />{t(label)}</button>
          ))}
        </div>
          ...
          <div>
            ...
            <GlassCard accent={trimesterColor} lift={false} style={{ padding: 24, marginBottom: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                <div>
                  <div style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "var(--text-3)", letterSpacing: "0.08em", marginBottom: 4 }}>{t("AAPKA WEEK")}</div>
                  <div style={{ fontSize: 36, fontWeight: 900, color: trimesterColor, fontFamily: "var(--font-display)", letterSpacing: "-0.03em" }}>{t("Week")} {week}</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "var(--text-3)", marginBottom: 4 }}>{t("TRIMESTER")}</div>
                  <div style={{ fontSize: 16, fontWeight: 700, color: trimesterColor }}>{t(trimester)}</div>
                </div>
              </div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "var(--text-3)", fontFamily: "var(--font-mono)", marginTop: 4 }}>
                <span>{t("Week 4")}</span><span>{t("Week 42")}</span>
              </div>
            </GlassCard>
              ...
              <div style={{ display: "flex", justifyContent: "space-around", marginTop: 8 }}>
                {[["Week 1-12", "Pehla"], ["Week 13-28", "Doosra"], ["Week 29-42", "Teesra"]].map(([wk, name]) => (
                  <div key={name} style={{ textAlign: "center" }}>
                    <div style={{ fontSize: 10, fontFamily: "var(--font-mono)", color: "var(--text-3)" }}>{t(name)}</div>
                    <div style={{ fontSize: 9, color: "var(--text-3)", fontFamily: "var(--font-mono)" }}>{t(wk)}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="m-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
              <GlassCard accent="#00E676" lift={false} style={{ padding: 18 }}>
                <div style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "#00E676", marginBottom: 8, letterSpacing: "0.06em", display: "flex", alignItems: "center", gap: 5 }}><Baby size={13} strokeWidth={1.8} />{t("BABY")}</div>
                <div style={{ fontSize: 13, color: "var(--text-2)", lineHeight: 1.6 }}>{t(data.baby)}</div>
              </GlassCard>
              <GlassCard accent="#00B4D8" lift={false} style={{ padding: 18 }}>
                <div style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "#00B4D8", marginBottom: 8, letterSpacing: "0.06em", display: "flex", alignItems: "center", gap: 5 }}><HeartPulse size={13} strokeWidth={1.8} />{t("AAPKA SHAREER")}</div>
                <div style={{ fontSize: 13, color: "var(--text-2)", lineHeight: 1.6 }}>{t(data.mother)}</div>
              </GlassCard>
            </div>

            <GlassCard accent="#00E676" lift={false} style={{ padding: 18, marginBottom: 12 }}>
              <div style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "var(--text-3)", marginBottom: 8, letterSpacing: "0.06em", display: "flex", alignItems: "center", gap: 5 }}><Lightbulb size={13} strokeWidth={1.8} />{t("IS WEEK KI TIP")}</div>
              <div style={{ fontSize: 14, color: "#F0F4FF", lineHeight: 1.6 }}>{t(data.tip)}</div>
            </GlassCard>

            <GlassCard accent="#FF4757" lift={false} style={{ padding: 18 }}>
              <div style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "#FF4757", marginBottom: 8, letterSpacing: "0.06em", display: "flex", alignItems: "center", gap: 5 }}><Siren size={13} strokeWidth={1.8} />{t("DANGER SIGN")}</div>
              <div style={{ fontSize: 13, color: "var(--text-2)", lineHeight: 1.6 }}>{t(data.warning)}</div>
              <a href="tel:102" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 7, marginTop: 12, background: "rgba(255,71,87,0.15)", border: "1px solid rgba(255,71,87,0.3)", borderRadius: 100, padding: "10px", textAlign: "center", color: "#FF4757", fontWeight: 700, textDecoration: "none", fontFamily: "var(--font-body)", fontSize: 14 }}><Phone size={15} strokeWidth={1.8} />{t("102 · Free Maternity Ambulance")}</a>
            </GlassCard>
            ...
            <button onClick={getAiAdvice} disabled={aiLoading} style={{ width: "100%", marginTop: 12, background: aiLoading ? "rgba(255,255,255,0.025)" : "linear-gradient(135deg,#00E676,#00C4FF)", border: aiLoading ? "1px solid var(--border)" : "none", borderRadius: 100, padding: "14px", color: aiLoading ? "var(--text-3)" : "#04060D", fontWeight: 600, cursor: aiLoading ? "not-allowed" : "pointer", fontFamily: "var(--font-body)", fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}>
              {aiLoading ? t("AI soch raha hai...") : <><Sparkles size={16} strokeWidth={1.8} />{`Week ${week} ki AI detail jaankari lo`}</>}
            </button>
            {aiAdvice && (
              <GlassCard accent="#00E676" lift={false} style={{ marginTop: 12, padding: 18 }}>
                <div style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "#00E676", marginBottom: 8, letterSpacing: "0.06em", display: "flex", alignItems: "center", gap: 5 }}><Sparkles size={13} strokeWidth={1.8} />{t("AI SALAH")}</div>
        ...
        {tab === "schemes" && (
          <div>
            <div style={{ marginBottom: 20, fontSize: 14, color: "var(--text-3)", lineHeight: 1.7 }}>
              {t("Yeh sarkari schemes aapke liye hain ·")} <strong style={{ color: "#00E676" }}>{t("bilkul free")}</strong>. {t("ASHA worker ya Anganwadi se apply karein.")}
            </div>
            {SCHEMES.map((scheme, i) => (
              <GlassCard key={i} accent="#00E676" lift={false} style={{ padding: 22, marginBottom: 12 }}>
                <div style={{ fontSize: 15, fontWeight: 700, color: "#F0F4FF", fontFamily: "var(--font-display)", marginBottom: 8 }}>{t(scheme.name)}</div>
                <div style={{ background: "rgba(0,230,118,0.08)", border: "1px solid rgba(0,230,118,0.15)", borderRadius: 10, padding: "10px 14px", marginBottom: 10 }}>
                  <span style={{ fontSize: 12, color: "#00E676", fontFamily: "var(--font-mono)", display: "inline-flex", alignItems: "center", gap: 4, verticalAlign: "middle" }}><IndianRupee size={13} strokeWidth={1.8} />{t("BENEFIT:")} </span>
                  <span style={{ fontSize: 13, color: "var(--text-2)" }}>{t(scheme.benefit)}</span>
                </div>
                <div style={{ fontSize: 12, color: "var(--text-3)", lineHeight: 1.6, display: "flex", alignItems: "flex-start", gap: 6 }}><FileText size={13} strokeWidth={1.8} style={{ flexShrink: 0, marginTop: 2 }} /><span>{t("Kaise apply karein:")} {t(scheme.how)}</span></div>
              </GlassCard>
            ))}
            <button onClick={() => router.push("/chat")} style={{ width: "100%", background: "linear-gradient(135deg,#00E676,#00C4FF)", border: "none", borderRadius: 100, padding: "14px", color: "#04060D", fontWeight: 600, cursor: "pointer", fontFamily: "var(--font-body)", fontSize: 14, marginTop: 8, display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}>
              <Sparkles size={16} strokeWidth={1.8} />{t("AI Se Scheme Ke Baare Mein Poochhein")}
            </button>
          </div>
        )}
        ...
        {tab === "danger" && (
          <div>
            <GlassCard accent="#FF4757" lift={false} style={{ padding: 22, marginBottom: 16 }}>
              <div style={{ fontSize: 15, fontWeight: 700, color: "#FF4757", marginBottom: 16, fontFamily: "var(--font-display)", display: "flex", alignItems: "center", gap: 8 }}><Siren size={18} strokeWidth={1.8} style={{ flexShrink: 0 }} />{t("IN Signs Pe TURANT 102 Call Karein")}</div>
              {["Bahut zyada bleeding · paani ki tarah beh raha ho", "Severe pet mein dard · ruk ruk ke ya lagaataar", "Baby ki movements bilkul band ho gayi hain", "Aankhon ke aage andhera ya double vision", "Face, haath, pair mein severe swelling", "Tej bukhaar 103°F se zyada", "Water break · liquid beh raha ho", "Severe sar dard jo band hi nahi ho raha"].map((sign, i) => (
                <div key={i} style={{ display: "flex", gap: 10, marginBottom: 10, fontSize: 13, color: "var(--text-2)", lineHeight: 1.5 }}>
                  <span style={{ color: "#FF4757", flexShrink: 0, fontWeight: 700 }}>{i + 1}.</span>{t(sign)}
                </div>
              ))}
              <a href="tel:102" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginTop: 16, background: "rgba(255,71,87,0.9)", border: "none", borderRadius: 100, padding: "14px", textAlign: "center", color: "#fff", fontWeight: 800, textDecoration: "none", fontFamily: "var(--font-body)", fontSize: 16, boxShadow: "0 0 24px rgba(255,71,87,0.3)" }}><Phone size={18} strokeWidth={1.8} />{t("102 ABHI CALL KAREIN · FREE")}</a>
            </GlassCard>
            <GlassCard accent="#fbbf24" lift={false} style={{ padding: 22 }}>
              <div style={{ fontSize: 15, fontWeight: 700, color: "#fbbf24", marginBottom: 12, fontFamily: "var(--font-display)", display: "flex", alignItems: "center", gap: 8 }}><AlertTriangle size={18} strokeWidth={1.8} style={{ flexShrink: 0 }} />{t("Yeh Signs Pe Aaj Doctor Se Milein")}</div>
              {["Halki bleeding ya brownish discharge", "Bahut zyada ulti · kuch bhi haazam nahi ho raha", "UTI ke signs · peeshab mein jalan ya baar baar aana", "Back pain jo zyada badh rahi ho", "Baby bump growth ruk gayi lagti ho"].map((sign, i) => (
                <div key={i} style={{ display: "flex", gap: 10, marginBottom: 8, fontSize: 13, color: "var(--text-2)", lineHeight: 1.5 }}>
                  <span style={{ color: "#fbbf24", flexShrink: 0 }}>•</span>{t(sign)}
                </div>
              ))}
            </GlassCard>
          </div>
