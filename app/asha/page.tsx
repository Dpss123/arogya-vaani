...
export default function AshaPage() {
  ...
  const getChecklist = async (visitType: string) => {
    ...
    try {
      ...
      if (!res.ok || data.error) setChecklist("Checklist nahi mil paayi. Dobara try karein.");
      else setChecklist(data.checklist || "Checklist nahi mil paayi.");
    } catch {
      setChecklist("Checklist nahi mil paayi. Dobara try karein.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      <div style={{ maxWidth: 820, margin: "0 auto", padding: "32px clamp(20px,4vw,40px) 100px" }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 24 }}>
          ...
          <div>
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--text-3)", letterSpacing: "0.08em", textTransform: "uppercase" }}>Home Visit + Vaccination Madad</div>
            <h1 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(26px,4vw,38px)", fontWeight: 800, letterSpacing: "-0.025em", color: "#F0F4FF", margin: "6px 0 0" }}>ASHA Assistant</h1>
          </div>
        </div>

        <div style={{ display: "flex", gap: 8, marginBottom: 22 }}>
          {([["checklist", FileText, "Visit Checklist"], ["vaccine", Syringe, "Vaccination"]] as const).map(([k, Icon, l]) => (
            <button key={k} onClick={() => setTab(k)} style={{ flex: 1, padding: "9px 4px", borderRadius: 100, border: `1px solid ${tab === k ? "rgba(0,230,118,0.35)" : "var(--border)"}`, background: tab === k ? "rgba(0,230,118,0.1)" : "transparent", color: tab === k ? "#00E676" : "var(--text-2)", fontSize: 13, cursor: "pointer", fontFamily: "var(--font-body)", fontWeight: tab === k ? 600 : 400, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6 }}><Icon size={14} color={tab === k ? "#00E676" : "var(--text-2)"} strokeWidth={1.8} />{l}</button>
          ))}
        </div>

        {tab === "checklist" && (
          <div>
            <div style={{ fontSize: 13, color: "var(--text-3)", marginBottom: 14, lineHeight: 1.6 }}>Visit ka type chunein · AI uski checklist banayega:</div>
            <div className="m-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 18 }}>
              {VISIT_TYPES.map(v => {
                ...
                <button key={v.key} onClick={() => getChecklist(v.key)} disabled={loading} style={{ background: visit === v.key ? "rgba(0,230,118,0.1)" : "rgba(255,255,255,0.025)", border: `1px solid ${visit === v.key ? "rgba(0,230,118,0.4)" : "var(--border)"}`, borderRadius: 14, padding: "16px 12px", cursor: loading ? "wait" : "pointer", textAlign: "left", fontFamily: "var(--font-body)" }}>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 700, color: "#F0F4FF" }}>{v.label}</div>
                </button>
                );
              })}
            </div>

            {loading && <div style={{ textAlign: "center", padding: 24, color: "var(--text-3)", fontFamily: "var(--font-mono)", fontSize: 13 }}>AI checklist bana raha hai...</div>}
            {checklist && !loading && (
              <GlassCard accent="#00E676" lift={false} style={{ padding: 22 }}>
                <div style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "#00E676", letterSpacing: "0.06em", marginBottom: 12, display: "inline-flex", alignItems: "center", gap: 6 }}><FileText size={13} color="#00E676" strokeWidth={1.8} />CHECKLIST</div>
        ...
        {tab === "vaccine" && (
          <div>
            <div style={{ fontSize: 13, color: "var(--text-3)", marginBottom: 14, lineHeight: 1.6 }}>India National Immunization Schedule (0–16 saal):</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {IMMUNIZATION.map((row, i) => (
                <GlassCard key={i} accent="#00B4D8" lift={false} style={{ padding: "14px 16px", borderRadius: 12 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: "#00B4D8", marginBottom: 5 }}>{row.age}</div>
            ...
            <GlassCard accent="#FF4757" lift={false} style={{ marginTop: 14, padding: "12px 16px", borderRadius: 12 }}>
              <div style={{ fontSize: 12, color: "var(--text-3)", lineHeight: 1.7, display: "flex", alignItems: "center", gap: 8 }}>
                ...
                Reference schedule. Apne PHC/ANM se local schedule confirm karein.
              </div>
            </GlassCard>
          </div>
        )}
