    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      <div style={{ maxWidth: 820, margin: "0 auto", padding: "32px clamp(20px,4vw,40px) 100px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
          ...
          <div>
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--text-3)", letterSpacing: "0.08em", textTransform: "uppercase" }}>{t("0–5 saal · WHO standards · screening")}</div>
            <h1 style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "clamp(26px,4vw,38px)", letterSpacing: "-0.025em", color: "#F0F4FF", margin: "5px 0 0" }}>{t("Child Growth Monitor")}</h1>
          </div>
        </div>
        ...
        <GlassCard accent="#00E676" lift={false} style={{ padding: 22, marginBottom: 20 }}>
          ...
          <div className="m-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
            <Field label={t("Umar (saal)")} value={ageY} onChange={setAgeY} placeholder={t("0-5")} />
            <Field label={t("+ Mahine")} value={ageM} onChange={setAgeM} placeholder={t("0-11")} />
          </div>
          <div className="m-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 18 }}>
            <Field label={t("Wazan (kg)")} value={weight} onChange={setWeight} placeholder={t("e.g. 12.5")} />
            <Field label={t("Lambai (cm)")} value={height} onChange={setHeight} placeholder={t("e.g. 87")} />
          </div>
          ...
          <div style={{ animation: "fadeUp 0.4s ease forwards" }}>
            <div className="m-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
              {([["Wazan (underweight)", result.underweight, `${result.weightPctMedian}% of WHO median`],
                ...
                <GlassCard key={label} accent={sevColor(sev)} lift={false} style={{ padding: 16 }}>
                  <div style={{ fontSize: 10, fontFamily: "var(--font-mono)", color: "var(--text-3)", letterSpacing: "0.04em", marginBottom: 8 }}>{t(label)}</div>
                  <div style={{ fontSize: 18, fontWeight: 800, color: sevColor(sev), fontFamily: "var(--font-display)" }}>{t(sevLabel(sev))}</div>
            ...
            {(result.underweight === "severe" || result.stunting === "severe") ? (
              <div style={{ display: "flex", alignItems: "flex-start", gap: 9, background: "#FF4757", color: "#fff", borderRadius: 14, padding: "13px 16px", fontWeight: 700, marginBottom: 14, fontSize: 14, lineHeight: 1.6 }}>
                ...
                {t("Bachche ko TURANT Anganwadi/health centre le jayein · yeh serious malnutrition ho sakta hai.")}
              </div>
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: 9, background: "rgba(251,191,36,0.12)", color: "#fbbf24", border: "1px solid rgba(251,191,36,0.3)", borderRadius: 14, padding: "12px 16px", fontWeight: 600, marginBottom: 14, fontSize: 13, lineHeight: 1.6 }}>
                ...
                {t("Jald Anganwadi/health centre le jayein aur nutrition par dhyan dein.")}
              </div>
            ) : null}
            ...
            {loading && <div style={{ textAlign: "center", padding: 16, color: "var(--text-3)", fontSize: 13, fontFamily: "var(--font-mono)" }}>{t("AI advice aa rahi hai...")}</div>}
            {advice && (
              <GlassCard accent="#00E676" lift={false} style={{ padding: 18, marginBottom: 14 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, fontFamily: "var(--font-mono)", color: "#00E676", letterSpacing: "0.06em", marginBottom: 10 }}><Utensils size={14} color="#00E676" strokeWidth={1.8} />{t("KYA KAREIN")}</div>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 9, background: "rgba(255,71,87,0.06)", border: "1px solid rgba(255,71,87,0.15)", borderRadius: 14, padding: "12px 16px", fontSize: 12, color: "var(--text-2)", lineHeight: 1.7 }}>
              ...
              {t("Yeh WHO median-based screening hai. Pakka assessment ke liye Anganwadi/ICDS centre par le jayein.")}
            </div>
          </div>
        )}
      </div>
