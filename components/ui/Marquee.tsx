...
export default function Marquee({ items, accent = "#00E676" }: { items: string[]; accent?: string }) {
  ...
  return (
    <div style={{ overflow: "hidden", padding: "14px 0", background: `${accent}0a`, borderTop: `1px solid ${accent}1f`, borderBottom: `1px solid ${accent}1f` }}>
      <div style={{ display: "flex", gap: 56, whiteSpace: "nowrap", width: "max-content", animation: "ticker 34s linear infinite" }}>
        {row.map((it, i) => (
